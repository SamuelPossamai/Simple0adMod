...
var playerIDs = [];
var firstHalfPlayerIDs = [];
var secondHalfPlayerIDs = [];

for(let i = 0; i < Math.floor(numPlayers/2); ++i) {
  firstHalfPlayerIDs.push(i + 1);
}

firstHalfPlayerIDs = sortPlayers(firstHalfPlayerIDs);

for(let i = Math.floor(numPlayers/2); i < numPlayers; ++i) {
  secondHalfPlayerIDs.push(i + 1);
}

secondHalfPlayerIDs = sortPlayers(secondHalfPlayerIDs);

for(let i = 0; i < numPlayers; ++i) {
  if(i & 1) {
    playerIDs.push(firstHalfPlayerIDs[Math.floor(i/2)]);
  }
  else {
    playerIDs.push(secondHalfPlayerIDs[Math.floor(i/2)]);
  }
}
...
var [playerPosition, playerAngle] = playerPlacementCustomAngle(
	...
	mapCenter,
	i => startAngle - Math.PI*(((i & 1) ? 1: 0)) - Math.PI * (Math.floor(i/2) + 1) / (numPlayers/2 + 1));
...
createAreas(
	...
	[avoidClasses(clIsland, 10, clPlayer, 6), stayClasses(clLand, 3)],
...
createAreas(
	...
	[avoidClasses(clIsland, 10, clHill, 15, clPlayer, 10), stayClasses(clLand, 7)],
...
for (let type of types)
	createAreas(
		...
		[avoidClasses(clPlayer, 5, clForest, 10, clHill, 0), stayClasses(clLand, 7)],
		num);
